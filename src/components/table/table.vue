<template>
  <div :class="wrapClasses" :style="styles">
    ...
    <div class="bee-table-resize-line" v-show="showResizeLine" ref="resizeLine"></div>
    ...
    computed: {
        ...
        fixedHeaderClasses() {
            return [
                ...
                {
                    [`${prefixCls}-fixed-header-with-empty`]: !this.rebuildData
                        .length
                }
            ];
        },
        styles() {
            ...
            if (this.height) {
                ...
                if (this.showPager) {
                    ...
                    if (this.height.constructor === String) {
        ...
        summaryData() {
            ...
            if (
                this.filterRows[0] !== undefined &&
                this.filterRows[0] !== null
            ) {
    ...
    methods: {
            ...
            if (this.showPager) {
                ...
                this.renderData =
                    skipNum + this.currentSize >= this.rebuildData.length
                        ? this.rebuildData.slice(
                              skipNum,
                              this.rebuildData.length
                          )
                        : this.rebuildData.slice(
                              skipNum,
                              skipNum + this.currentSize
                          );
            } else {
        ...
        handleResize() {
            ...
            for (let i = 0; i < this.filterRows.length; i++) {
                ...
                let width =
                    columnWidth + (column.minWidth ? column.minWidth : 0);
                if (column.width) {
                    width = column.width;
                } else {
                    if (column._width) {
        ...
        handleSort(_index, type) {
            ...
            this.$emit('on-sort-change', {
                column: JSON.parse(
                    JSON.stringify(
                        this.allColumns[this.cloneColumns[index]._index]
                    )
                ),
                key: key,
                order: type
        ...
        filterData(data, column) {
            if (
                ...
                if (column._type === 'number' || column._type === 'string') {
                    ...
                    if (
                        column._dateType === undefined ||
                        column._dateType === null
                    ) {
                        return data;
                    }
                    ...
                    if (
                        ...
                        if (
                            column._filterValue === null ||
                            column._filterValue === undefined ||
                            column._filterValue.replace(/(^s*)|(s*$)/g, '')
                                .length === 0
                        ) {
                            return data;
                        }
        ...
        handleFilter(index) {
            ...
            let filterData = this.makeDataWithSort(
                JSON.parse(this.originalData)
            );

            filterData = this.filterOtherData(filterData, index);
            this.rebuildDataStr = JSON.stringify(
                this.filterData(filterData, column)
            );
            this.getRenderData();
        ...
        makeColumns(cols) {
            ...
            columns.forEach((column, index) => {
                ...
                column._type =
                    column.type === undefined ? 'string' : column.type;
        ...
        exportXlsxJson({ fileName, header, headerDisplay, rows, formats }) {
            ...
            if (header !== null && header !== undefined) {
                ...
                for (let i = 0; i < this.columns.length; i++) {
                    defaultHeader.push(this.columns[i].key);
                    defaultDisplayHeader[this.columns[i].key] = this.columns[
                        i
                    ].title;
                }
            }
            ...
            for (let i = 0; i < this.columns.length; i++) {
                if (defaultHeader.some(p => p === this.columns[i].key)) {
                    if (
                        !defaultDisplayHeader.hasOwnProperty(
                            this.columns[i].key
                        )
                    ) {
            ...
            for (let m = 0; m < this.rebuildData.length; m++) {
                let row = {};
                for (let n = 0; n < defaultHeader.length; n++) {
                    row[defaultHeader[n]] = this.rebuildData[m][
                        defaultHeader[n]
                    ];
                    ...
                    if (formats !== null && formats !== undefined) {
                        if (
                            ...
                            try {
                                row[defaultHeader[n]] = formats[
                                    defaultHeader[n]
                                ](
                                    this.rebuildData[m],
                                    this.columns.find(
                                        p => p.key === defaultHeader[n]
                                    ),
                                    this.rebuildData[m][defaultHeader[n]]
                                );
                            } catch (e) {
                                console.log(
                                    'format method of defaultHeader[n] not valid',
                                    e
                                );
                            }
                        }
        ...
        initColumns() {
            ...
            this.leftFixedFilterRows = this.makeFilterRows(
