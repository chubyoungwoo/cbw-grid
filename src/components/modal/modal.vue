        ...
        <div :class="classes" :style="mainStyles" v-show="visible" @mousedown="handleMousedown">
          ...
          >
            <a :class="[prefixCls + '-close']" v-if="closable" @click="close">
              <slot name="close">
                <Icon customPrefixCls customIconCls="bee-sys-icon" type="ios-close"></Icon>
              </slot>
            </a>
...
export default {
  ...
  props: {
    ...
    maskClosable: {
      type: Boolean,
      default() {
        return !this.$BEETABLE || this.$BEETABLE.modal.maskClosable === ""
          ? true
          : this.$BEETABLE.modal.maskClosable;
      }
    },
    title: {
      type: String
