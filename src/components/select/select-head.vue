<template>
  <div @click="onHeaderClick" :class="headCls">
    <span :class="[prefixCls + '-prefix']" v-if="$slots.prefix || prefix">
      <slot name="prefix">
        <Icon customPrefixCls customIconCls="bee-sys-icon" :type="prefix" v-if="prefix" />
      </slot>
    </span>
    <div
      class="bee-tag bee-tag-checked"
      v-for="(item, index) in selectedMultiple"
      v-if="maxTagCount === undefined || index < maxTagCount"
    >
      <span class="bee-tag-text">
        {{
    ...
    >
      <span class="bee-tag-text bee-select-max-tag">
            ...
            return [
                {
                    [prefixCls + '-head-with-prefix']:
                        this.$slots.prefix || this.prefix,
                    [prefixCls + '-placeholder']:
                        showPlaceholder && !filterable,
    ...
    methods: {
        ...
        handleInputDelete(e) {
            ...
            if (
                ...
                this.removeTag(
                    this.selectedMultiple[this.selectedMultiple.length - 1]
                );
            }
        },
        handleInputEnter() {
